// expect: idxSet/sloppy = 1,9~idxSet/strict = 1,9~in/sloppy = true,true,false,true,true,true,true,true~in/strict = true,true,false,true,true,true,true,true~hasOwn/sloppy = true,true,false,true,true,true,true~hasOwn/strict = true,true,false,true,true,true,true~gopd/sloppy = v=1WEC,v=1WEC,v=1WC,v=fnWC,v=fnWC~gopd/strict = v=1WEC,v=1WEC,v=1WC,acc,v=fnWC~keys/sloppy = 0,1,x,y|0,1,length,callee,x,y|2|0,1,length,callee,x,y,Symbol(Symbol.iterator),Symbol(s)~keys/strict = 0,1,x,y|0,1,length,callee,x,y|2|0,1,length,callee,x,y,Symbol(Symbol.iterator),Symbol(s)~defProp/sloppy = 5,7,0,y,v=5E~defProp/strict = 5,7,0,y,v=5E~forIn/sloppy = 0,1,x~forIn/strict = 0,1,x~spread/sloppy = {"0":1,"x":1},2~spread/strict = {"0":1,"x":1},2~assignSrc/sloppy = {"0":1,"x":1}~assignSrc/strict = {"0":1,"x":1}~reflect/sloppy = true,3,true,true,8,true,false~reflect/strict = true,3,true,true,8,true,false~del/sloppy = true,,false,true,true,false~del/strict = true,,false,true,true,false~freeze/sloppy = no-throw,1,no-throw,1,true,false,v=1E,v=1E~freeze/strict = no-throw,1,no-throw,1,true,false,v=1E,v=1E~seal/sloppy = 2,true,false,false,v=2WE~seal/strict = 2,true,false,false,v=2WE~noext/sloppy = no-throw,,TypeError,false,false~noext/strict = no-throw,,TypeError,false,false~lengthSet/sloppy = 5,v=5WC,,false~lengthSet/strict = 5,v=5WC,,false~mapped/sloppy = 5,6,0,x~mapped/strict = 1,5,0,x~json/sloppy = {"0":1,"1":"b","x":1}~json/strict = {"0":1,"1":"b","x":1}~entries/sloppy = [["0",1],["x","q"]][1,"q"]~entries/strict = [["0",1],["x","q"]][1,"q"]
// skip-typecheck
// paserati#535: an arguments object's named own properties used to live in a
// private map only dot access knew about - bracket writes, in, hasOwn,
// descriptors, keys, defineProperty, for-in, spread, Object.assign, Reflect,
// delete, freeze/seal/preventExtensions and JSON all missed them. Every
// consumer now goes through ArgumentsOwnProperty (which also resolves
// length and callee, honoring redefinition/deletion) and keys come out in
// [[OwnPropertyKeys]] order. Each row matches Node, for sloppy and strict
// arguments objects.
